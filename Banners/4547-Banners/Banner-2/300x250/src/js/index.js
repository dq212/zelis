...
(function () {
	...
	nameSpace.init = function () {
		...
		click_through.onmouseover = function () {
			...
			TweenMax.set('#cta-copy-2', { y: 30, x: 0 });
		...
		TweenMax.set(['#hero', '#copy-1', '#copy-1b', '#copy-2'], { autoAlpha: 0 });
		TweenMax.set(['#copy-1', '#copy-1b', '#copy-2'], { x: -width });
		// TweenMax.set(['#hero'], { x: width });
	};
	...
	nameSpace.initAnimation = function () {
		...
		timeline
			.to('#sign', 0.5, { scale: 0.5, transfromOrigin: 'center center', x: -62, y: -81 }, '+=2')
			.to('#copy-1', 0.8, { autoAlpha: 1, x: 0, z: 0.01, transformPerspective: 400, rotationZ: 0.01, force3D: true })
			.to('#copy-1b', 0.8, { autoAlpha: 1, x: 0, z: 0.01, transformPerspective: 400, rotationZ: 0.01, force3D: true }, "+=1")
			.to(['#copy-1', '#copy-1b'], 0.5, { autoAlpha: 0, x: width, z: 0.01, transformPerspective: 400, rotationZ: 0.01, force3D: true }, "+=2")
			.to(['#copy-2'], 0.5, { z: 0.01, transformPerspective: 400, rotationZ: 0.01, force3D: true, autoAlpha: 1, x: 0 })
			.to(['#hero'], 0.3, { z: 0.01, transformPerspective: 400, rotationZ: 0.01, force3D: true, autoAlpha: 1 }, '-=0.4')
			.to(['.logo-purple', '.logo-yellow', '.logo-orange'], 0.3, { css: { fill: '#ffffff' } }, "-=0.4")
